...
def load_ratings(ratings_file: str, sep: str = None) -> pd.DataFrame:
    ...
    for s in try_seps:
        try:
            df = pd.read_csv(ratings_file, sep=s, engine="python", header=None, comment="#",
    ...
    raise ValueError(f"Could not read ratings file {ratings_file}. Last error: {last_exc}")


def validate_ratings_df(df: pd.DataFrame):
    required = {"user_id", "item_id", "rating", "timestamp"}
    if not required.issubset(set(df.columns)):
        raise ValueError(f"Ratings dataframe missing required columns: {required - set(df.columns)}")
...
def filter_by_interactions(df: pd.DataFrame, min_user_interactions: int = 1, min_item_interactions: int = 1) -> pd.DataFrame:
    logger.info(f"Filtering users with < {min_user_interactions} interactions and items with < {min_item_interactions} interactions")
    # filter users
    ...
    logger.info(f"After filtering: interactions={len(df)}, users={df['user_id'].nunique()}, items={df['item_id'].nunique()}")
    return df
...
def apply_mappings(df: pd.DataFrame, user2idx: Dict[int, int], item2idx: Dict[int, int]) -> pd.DataFrame:
    df = df.copy()
    df["user_idx"] = df["user_id"].map(user2idx)
    df["item_idx"] = df["item_id"].map(item2idx)

    if df["user_idx"].isnull().any() or df["item_idx"].isnull().any():
        raise ValueError("Some user_id/item_id could not be mapped to index after building mappings.")
    df["user_idx"] = df["user_idx"].astype(int)
    df["item_idx"] = df["item_idx"].astype(int)
    return df


def leave_one_out_split(df: pd.DataFrame, min_for_val: int = 2) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
...
def save_processed_splits(out_dir: str, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame):
    os.makedirs(out_dir, exist_ok=True)
    train_path = os.path.join(out_dir, "train.csv")
    val_path = os.path.join(out_dir, "val.csv")
    test_path = os.path.join(out_dir, "test.csv")
    # save with columns: user_idx,item_idx,rating,timestamp, (optionally original ids)
    save_cols = ["user_idx", "item_idx", "rating", "timestamp", "user_id", "item_id"]
...
def preprocess_from_config(cfg: dict, write_config_back: bool = False) -> dict:
    ...
    ml100k_dir = data_cfg.get("ml100k_dir", os.path.dirname(ratings_file) if ratings_file else "data/ml-100k")
    ...
    ratings = filter_by_interactions(ratings, min_user_interactions, min_item_interactions)
    ...
    if write_config_back:
        ...
        if cfg_path is None:
            logger.warning("write_config_back requested but original config path not set; skipping writing to config.")
        else:
            try:
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess MovieLens-100k for recsys project")
