...
def parse_args():
    ...
    parser.add_argument(
        "--write-config",
        action="store_true",
        help="If set, write detected n_users/n_items back into config after preprocessing",
    )
    parser.add_argument(
        "--resume",
        type=str,
        default=None,
        help="Path to model dir to resume from (load pretrained weights)",
    )
    parser.add_argument(
        "--no-save",
        action="store_true",
        help="Do not save final model (useful for quick tests)",
    )
    parser.add_argument(
        "--override",
        type=str,
        nargs="*",
        default=[],
        help="Override config values, format key=value (dot notation for nested). Example: train.epochs=5 train.lr=0.0005",
    )
...
def build_optimizer(model: torch.nn.Module, optim_cfg: Dict[str, Any]):
    ...
    logger.info(
        f"Built optimizer {name} lr={lr_val} betas={betas} eps={eps} weight_decay={weight_decay}"
    )
    return opt
...
def main():
    ...
    if args.preprocess:
        ...
        meta = preprocess_mod.preprocess_from_config(
            cfg, write_config_back=args.write_config
        )
    ...
    val_path = cfg.get("data", {}).get("val_path") or os.path.join(
        processed_subdir, "val.csv"
    )
    test_path = cfg.get("data", {}).get("test_path") or os.path.join(
        processed_subdir, "test.csv"
    )
    ...
    if (n_users is None) or (n_items is None):
        ...
        if os.path.exists(train_path):
            ...
            if "user_idx" in df_all.columns and "item_idx" in df_all.columns:
                ...
                logger.info(
                    f"Inferred n_users={n_users}, n_items={n_items} from processed files."
                )
        else:
            logger.warning(
                "Processed train/test files not found and model_init_args.n_users/n_items not set. Preprocess first or fill these in config."
            )
    ...
    if args.resume:
        ...
        try:
            ...
            model_loaded, _ = model.__class__.from_pretrained(
            ...
            logger.exception(
                f"Failed to load pretrained weights from {args.resume}: {e}"
            )

    # optimizer & scheduler
    optim_cfg = cfg.get(
        "optimizer",
        {"name": "adam", "params": {"lr": train_cfg.get("lr", 1e-3)}},
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_res = engine.train_epoch(
            ...
            use_amp=use_amp,
            max_grad_norm=(
                float(train_cfg.get("max_grad_norm"))
        ...
        t1 = time.time()
        logger.info(
            f"Epoch {epoch} train loss: {train_res['loss']:.6f}, examples={train_res['examples']}, time={(t1-t0):.1f}s"
        )
        ...
        if dataloaders.val is not None:
            ...
            eval_full_batch = int(
                cfg.get("evaluation", {}).get("full_ranking_batch", 4096)
            )
            ...
            if val_rmse is not None and val_rmse < best_val_rmse:
                ...
                if not args.no_save:
                    logger.info(
                        f"New best val RMSE {best_val_rmse:.6f} at epoch {epoch}. Saving best model."
                    )
    ...
    if dataloaders.test is not None:
        test_metrics = engine.evaluate(
            ...
            device=device,
            model_type=model_type,
            compute_full_ranking=bool(
                cfg.get("metrics", {}).get("compute_full_ranking", False)
            ),
            topk=int(cfg.get("metrics", {}).get("eval_topk", 10)),
            n_items=dataloaders.n_items,
            eval_full_batch_size=int(
                cfg.get("evaluation", {}).get("full_ranking_batch", 4096)
            ),
        )
